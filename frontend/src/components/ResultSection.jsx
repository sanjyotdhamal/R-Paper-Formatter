...
function ResultSection({ file, format, downloadUrl }) {
  ...
  const handleDownload = () => {
    if (downloadUrl) {
      ...
      link.download = 'formatted_' + format + '.docx';
      link.click();
    }
  };
  ...
  return (
    <div className="mb-8">
      ...
      <div className={`rounded-2xl border-2 p-8 ${darkMode ? 'border-gray-700 bg-gray-800' : 'border-gray-200 bg-white shadow-sm'}`}>
        <div className="text-center mb-6">
          ...
          <p className="text-gray-400 text-sm">
            {file} formatted to <span className="text-blue-600 font-bold">{format}</span>
          </p>
        </div>
        <div className={`rounded-xl p-6 mb-6 ${darkMode ? 'bg-gray-700' : 'bg-gray-50'}`}>
          <p className="text-blue-600 font-semibold tracking-widest text-xs uppercase mb-3">
            Preview
          </p>
          <div className={`rounded-lg p-6 ${darkMode ? 'bg-gray-600' : 'bg-white shadow-sm'}`}>
            <p className={`font-black text-center text-base mb-1 ${darkMode ? 'text-white' : 'text-gray-900'}`}>
        ...
        <div className="text-center">
          <button
            ...
            className="bg-blue-600 text-white px-10 py-3 rounded-xl font-black hover:bg-blue-700 transition-all hover:scale-105 shadow-lg flex items-center gap-2 mx-auto"
          >
            <Download size={18} />
